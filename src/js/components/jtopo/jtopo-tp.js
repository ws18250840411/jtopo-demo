 ...
 * jtopo优化版
 *
 * 作者之前没有开放源码，根据具体需求，修改部分代码以支持新功能
 * 增加部分注释
 *
 */
...
!function (window) {
     ...
     * @param height 高度
     * @param t_left 左顶点偏移量
     * @param t_right 右顶点偏移量
     * @constructor
     */
    CanvasRenderingContext2D.prototype.JTopoTrapezoid = function (x, y, width, height, t_left, t_right,lineWidth,lineStyle,lineDash,lineDashWidth,lineDashStyle) {
        "undefined" == typeof lineWidth && (lineWidth = 2);
        "undefined" == typeof lineStyle && (lineStyle = "#000");
        "undefined" == typeof t_left && (t_left = 0);
        "undefined" == typeof t_right && (t_right = t_left);
        if(!lineDash){
            this.strokeStyle = lineStyle;//针对stroke()有效的颜色
            this.lineWidth = lineWidth;//线段的宽度
            this.beginPath();
            this.moveTo(x + t_left, y);
            this.lineTo(x + width - t_right, y);
            this.lineTo(x + width, y + height);
            this.lineTo(x, y + height);
            this.lineTo(x + t_left, y);
            this.stroke();
            this.closePath();
        } else {
            this.JTopoTrapezoidDash(x, y, width, height, t_left, t_right,lineDashWidth,lineDashStyle);
        }
    };
     ...
     * @constructor
     */
    CanvasRenderingContext2D.prototype.JTopoTrapezoidDash = function (x, y, width, height, t_left, t_right,lineDashWidth,lineDashStyle) {
        "undefined" == typeof lineDashWidth && (lineDashWidth = 2);
        "undefined" == typeof lineDashStyle && (lineDashStyle = "#000");
        "undefined" == typeof t_left && (t_left = 0);
        "undefined" == typeof t_right && (t_right = t_left);
        this.strokeStyle = lineDashStyle;//针对stroke()有效的颜色
        this.lineWidth = lineDashWidth;//线段的宽度
        this.setLineDash([6,2]);//设置虚线，数组里面的第一个参数是实现的宽度，第二个参数是空白的宽度
        this.beginPath();
        this.moveTo(x + t_left, y);
        this.lineTo(x + width - t_right, y);
        this.lineTo(x + width, y + height);
     ...
     * @constructor
     */
    CanvasRenderingContext2D.prototype.JTopoRoundRect = function (x, y, c, d, e) {
        "undefined" == typeof e && (e = 5);
        this.beginPath(), this.moveTo(x + e, y);
        this.lineTo(x + c - e, y);
        this.quadraticCurveTo(x + c, y, x + c, y + e);
        this.lineTo(x + c, y + d - e);
        this.quadraticCurveTo(x + c, y + d, x + c - e, y + d);
        this.lineTo(x + e, y + d);
        this.quadraticCurveTo(x, y + d, x, y + d - e);
        this.lineTo(x, y + e);
        this.quadraticCurveTo(x, y, x + e, y);
        this.closePath();
    };
    ...
    function f(b, c, g) {
        ...
        this.initialize = function (b, c, d) {
            ...
            if (null != this.lineEndPointOffset) {
                //b[1].x -= this.lineEndPointOffset;
            }
            if (null != this.lineStartPointOffset) {
                //b[0].x -= this.lineStartPointOffset;
            }
            if (this.nodeA === this.nodeZ)return void this.paintLoop(a);
            if (null != this.arrowType && 'circle' === this.arrowType) {
                a.beginPath();
                a.fillStyle = "rgba(" + this.strokeColor + "," + this.alpha + ")";
                a.arc(b[0].x + 2 * this.arrowsRadius, b[0].y, this.arrowsRadius, 0, 2 * Math.PI);
                a.closePath();
            } else if (null != this.arrowType && 'block' === this.arrowType) {
                a.beginPath();
                a.fillStyle = "rgba(" + this.strokeColor + "," + this.alpha + ")";
                a.moveTo(b[0].x + 2 * this.arrowsRadius, b[0].y);
                a.fillRect(b[0].x, b[0].y - this.arrowsRadius, 2 * this.arrowsRadius, 2 * this.arrowsRadius);
            } else if (null != this.arrowType && 'hollow' === this.arrowType) {
                a.beginPath();
                a.fillStyle = "rgba(" + this.strokeColor + "," + this.alpha + ")";
                a.moveTo(b[0].x + 2 * this.arrowsRadius, b[0].y);
                a.strokeRect(b[0].x, b[0].y - 0.75 * this.arrowsRadius, 1.5 * this.arrowsRadius, 1.5 * this.arrowsRadius);
            } else if (null != this.arrowType && 'hollowBlock' === this.arrowType) {
                a.beginPath();
                a.fillStyle = "rgba(" + this.strokeColor + "," + this.alpha + ")";
                a.moveTo(b[0].x + 2 * this.arrowsRadius, b[0].y);
                a.strokeRect(b[0].x, b[0].y - 0.75 * this.arrowsRadius, 1.5 * this.arrowsRadius, 1.5 * this.arrowsRadius);
            } else if(null != this.arrowType && 'flexionalLinks' === this.arrowType) {
                a.beginPath();
                a.fillStyle = "rgba(" + this.strokeColor + "," + this.alpha + ")";
                a.moveTo(b[0].x, b[0].y);
                a.moveTo(b[0].x, b[0].y-3*this.arrowsRadius);
                a.strokeRect(b[0].x-0.5*this.arrowsRadius, b[0].y-3*this.arrowsRadius, this.arrowsRadius,this.arrowsRadius),
                //a.strokeRect(b[0].x, b[0].y, this.arrowsRadius,this.arrowsRadius),
                a.closePath()
            } else {
                a.beginPath();
                a.moveTo(b[0].x, b[0].y);
            ...
            for (var c = 1; c < b.length; c++){
                //if(c == 3){
                //    b[c].y = b[c].y +3*this.arrowsRadius;
                //}
            ...
            var e = this.arrowsOffset,
                ...
                m = h.x + e * Math.cos(i), n = h.y + e * Math.sin(i);
            i -= Math.PI / 2;
            ...
            if (null != this.arrowType && 'circle' === this.arrowType) {
                b.beginPath(),
                    ...
                    b.arc(o.x - f + 2 * this.arrowsRadius + 1, o.y + f, this.arrowsRadius, 0, 2 * Math.PI),
                    b.stroke(),
                    b.closePath()

            } else if (null != this.arrowType && 'block' === this.arrowType) {
                b.fillStyle = "rgba(" + this.strokeColor + "," + this.alpha + ")",
                    //b.arc(o.x-f, o.y + f, this.arrowsRadius, 0, 2 * Math.PI),
                    b.fillRect(o.x - f + this.arrowsRadius, o.y + f - this.arrowsRadius, 2 * this.arrowsRadius, 2 * this.arrowsRadius),
                    b.stroke(),
                    b.closePath()
            } else if (null != this.arrowType && 'hollow' === this.arrowType) {
                b.strokeRect(o.x - f + 2 * this.arrowsRadius, o.y + f - 0.75 * this.arrowsRadius, 1.5 * this.arrowsRadius, 1.5 * this.arrowsRadius),
                    b.stroke(),
                    b.closePath()
            } else if (null != this.arrowType && 'hollowBlock' === this.arrowType) {
                b.fillRect(o.x - f + this.arrowsRadius, o.y + f - this.arrowsRadius, 2 * this.arrowsRadius, 2 * this.arrowsRadius),
                    ...
                    b.beginPath();
                    b.fillStyle = "rgba(" + this.strokeColor + "," + this.alpha + ")",
                    b.strokeRect(o.x, o.y-2*this.arrowsRadius, this.arrowsRadius,this.arrowsRadius),
                    b.stroke(),
                    b.closePath()
            } else {
                //画箭头部分
    ...
    function i(a, b, c) {
        this.initialize = function () {
            ...
            if (this.nodeA === this.nodeZ)return void this.paintLoop(a);
            a.beginPath(), a.moveTo(b[0].x - 10, b[0].y);
            for (var c = 1; c < b.length; c++) {
                var d = b[c - 1], e = b[c], f = (d.x + e.x) / 2, g = (d.y + e.y) / 2;
                g += (e.y - d.y) / 2, a.strokeStyle = "rgba(" + this.strokeColor + "," + this.alpha + ")",
                    a.lineWidth = this.lineWidth,
                    a.moveTo(d.x, d.cy),
                    a.quadraticCurveTo(f, g, e.x, e.y), a.stroke()
            }
            if (a.stroke(), a.closePath(), null != this.arrowsRadius) {
    ...
    function b(c) {
        this.initialize = function (c) {
            b.prototype.initialize.apply(this, null), this.elementType = "container",
                this.zIndex = a.zIndex_Container,
                this.width = 100, this.height = 100,
                this.childs = [], this.alpha = .5,
                this.dragable = !0, this.childDragble = !0,
                this.visible = !0, this.fillColor = "10,100,80",
                this.borderWidth = 0, this.borderColor = "255,255,255",
                this.borderRadius = null, this.font = "12px Consolas",
                this.fontColor = "255,255,255", this.text = c,
                this.textPosition = "Bottom_Center", this.textOffsetX = 0,
                this.textOffsetY = 0, this.layout = new a.layout.AutoBoundLayout
            ...
            this.visible && (this.layout && this.layout(this, this.childs),
                a.beginPath(), a.fillStyle = "rgba(" + this.fillColor + "," + this.alpha + ")",
                //a.strokeStyle="red", a.lineWidth=5,
                ...
                null == this.borderRadius || 0 == this.borderRadius ? a.JTopoTrapezoid(this.x, this.y, this.width, this.height,  this.TrapezoidTopLeftPadding,this.TrapezoidTopRightPadding,this.lineWidth,this.lineStyle,this.lineDash,this.lineDashWidth,this.lineDashStyle) : a.JTopoRoundRect(this.x, this.y, this.width, this.height, this.borderRadius),
                a.fill(), a.closePath(), this.paintText(a), this.paintBorder(a))
        }, this.paintBorder = function (a) {
            if (0 != this.borderWidth) {
                a.beginPath(), a.lineWidth = this.borderWidth, a.strokeStyle = "rgba(" + this.borderColor + "," + this.alpha + ")";
                var b = this.borderWidth / 2;
